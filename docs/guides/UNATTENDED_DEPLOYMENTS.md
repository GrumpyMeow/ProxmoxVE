...
```bash
# Test GitHub access
curl -I https://raw.githubusercontent.com/GrumpyMeow/ProxmoxVE/main/ct/debian.sh
...
**Simplest form:**
```bash
var_hostname=myserver bash -c "$(curl -fsSL https://raw.githubusercontent.com/GrumpyMeow/ProxmoxVE/main/ct/debian.sh)"
```
...
var_protection=yes \
var_verbose=no \
  bash -c "$(curl -fsSL https://raw.githubusercontent.com/GrumpyMeow/ProxmoxVE/main/ct/debian.sh)"
...
```bash
bash -c "$(curl -fsSL https://raw.githubusercontent.com/GrumpyMeow/ProxmoxVE/main/ct/pihole.sh)"
...
var_hostname=transcoder \
  bash -c "$(curl -fsSL https://raw.githubusercontent.com/GrumpyMeow/ProxmoxVE/main/ct/plex.sh)"
```
...
for app in "${apps[@]}"; do
  ...
  var_cpu=2 \
  var_ram=2048 \
    bash -c "$(curl -fsSL https://raw.githubusercontent.com/GrumpyMeow/ProxmoxVE/main/ct/${app}.sh)"
...
for app in "${!CONTAINERS[@]}"; do
  ...
  var_tags="$tags,automated" \
    bash -c "$(curl -fsSL https://raw.githubusercontent.com/GrumpyMeow/ProxmoxVE/main/ct/${app}.sh)" 2>&1 | tee "deploy-${app}.log"
  ...
  tasks:
    ...
    - name: Deploy containers
      shell: |
        ...
        bash -c "$(curl -fsSL https://raw.githubusercontent.com/GrumpyMeow/ProxmoxVE/main/ct/{{ item.name }}.sh)"
      args:
        executable: /bin/bash
      loop: "{{ containers }}"
...
resource "null_resource" "deploy_container" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "var_net=dhcp",
      "bash -c \"$(curl -fsSL https://raw.githubusercontent.com/GrumpyMeow/ProxmoxVE/main/ct/${each.value.template}.sh)\""
    ]
      ...
      - name: Deploy to Proxmox
        ...
        with:
          ...
          script: |
            ...
            bash -c "$(curl -fsSL https://raw.githubusercontent.com/GrumpyMeow/ProxmoxVE/main/ct/${{ github.event.inputs.container_type }}.sh)"
...
deploy_container() {
  ...
  var_net=dhcp \
  var_ssh=yes \
    bash -c "$(curl -fsSL https://raw.githubusercontent.com/GrumpyMeow/ProxmoxVE/main/ct/${APP}.sh)" 2>&1 | tee deploy.log
...
deploy_secure() {
  ...
  var_verbose=no \
    bash -c "$(curl -fsSL https://raw.githubusercontent.com/GrumpyMeow/ProxmoxVE/main/ct/${APP}.sh)" 2>&1 | tee -a "$LOG_FILE"
