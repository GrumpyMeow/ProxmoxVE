...
# bash -c "$(curl -fsSL https://raw.githubusercontent.com/GrumpyMeow/ProxmoxVE/main/tools/pve/cron-update-lxcs.sh)"
...
add() {
  ...
  sh -c '(crontab -l -u root 2>/dev/null; echo "0 0 * * 0 PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin /bin/bash -c \"\$(curl -fsSL https://raw.githubusercontent.com/GrumpyMeow/ProxmoxVE/main/tools/pve/update-lxcs-cron.sh)\" >>/var/log/update-lxcs-cron.log 2>/dev/null") | crontab -u root -'
  clear
